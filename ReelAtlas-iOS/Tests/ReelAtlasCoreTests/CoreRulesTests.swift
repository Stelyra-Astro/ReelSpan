...
final class MovieMetadataServiceTests: XCTestCase {
    ...
    func testServiceClearCacheForcesNextMetadataRequestToUseTransport() async throws {
        let disk = cache()
        try disk.writeMetadata(.fixture(id: 550), tmdbID: 550)
        let transport = StaticHTTPTransport(
            status: 200,
            data: try JSONEncoder().encode(MovieMetadata.fixture(id: 550))
        )
        let service = MovieMetadataService(transport: transport, cache: disk)
        ...
        _ = try await service.metadata(tmdbID: 550)
        let refreshedRequestCount = await transport.requests.count
        XCTAssertEqual(refreshedRequestCount, 1)
    }
