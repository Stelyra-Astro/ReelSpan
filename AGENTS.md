...
- 2026-09-17 12:26:11 已成功上传缓存修复版本 1.0 (13)，日志确认 `Upload succeeded` / `EXPORT SUCCEEDED`、退出码 0，当时 Apple 正在处理，未提交审核。下一次新发布归档从 Build 14 开始；发布前核对 App Store Connect 最新编号，避免重复。
...
- Build 13 使用最终归档 `Artifacts/silent-refresh-validation/build13/ReelSpan-Build-13-final.xcarchive`，App/dSYM UUID `2C134787-99B6-3DFE-925E-D4C9E9BE828C`；显式 API Key 导出复用了 Store profile 与 Apple Distribution 证书。上传证据在该目录 `upload.log`，处理完成状态需另查，不能重复上传。
