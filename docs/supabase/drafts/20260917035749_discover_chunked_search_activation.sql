-- Repair full-field flags from preparation before activating exact-title scoring.
CREATE OR REPLACE FUNCTION reelspan_private.refresh_search_chunks() RETURNS trigger
...
UPDATE reelspan_private.search_chunks SET is_full_field=false WHERE chunk_no<>1 AND is_full_field;
CREATE INDEX search_chunks_text_trgm ON reelspan_private.search_chunks USING gin(chunk_text extensions.gin_trgm_ops);
CREATE INDEX search_chunks_literal_pairs ON reelspan_private.search_chunks USING gin(reelspan_private.literal_pair_terms(chunk_text));
DROP TRIGGER reelspan_movie_search_refresh ON public.movies;
DROP FUNCTION reelspan_private.refresh_movie_search();
...
WITH input AS NOT MATERIALIZED (SELECT lower(trim(left(coalesce($1,''),80))) q,greatest(1,least(coalesce($9,20),50)) page_size,greatest(0,least(coalesce($10,0),100000)) page_start, '%' || replace(replace(replace(lower(trim(left(coalesce($1,''),80))), chr(92), chr(92)||chr(92)), '%', chr(92)||'%'), '_', chr(92)||'_') || '%' pattern),
...
chunk_hits AS MATERIALIZED (
 SELECT ch.owner_kind,ch.owner_id,
 max(CASE WHEN ch.score=110 AND ch.is_full_field AND ch.chunk_text=i.q THEN 120 ELSE ch.score END)::integer score
 FROM reelspan_private.search_chunks ch CROSS JOIN input i
 WHERE i.q<>'' AND (
 (length(i.q)>=3 AND (i.q ~ '[[:alnum:]]{3}' OR reelspan_private.literal_pair_terms(ch.chunk_text) @> ARRAY[left(i.q,2)]) AND ch.chunk_text LIKE i.pattern ESCAPE chr(92))
 OR (length(i.q)<3 AND reelspan_private.literal_pair_terms(ch.chunk_text) @> ARRAY[i.q])
 )
 GROUP BY ch.owner_kind,ch.owner_id
), metadata_matches AS MATERIALIZED (SELECT owner_id::bigint tmdb_id,score FROM chunk_hits WHERE owner_kind='m'),
index_matches AS MATERIALIZED (SELECT owner_id movie_qid,score FROM chunk_hits WHERE owner_kind='s'), relevance AS MATERIALIZED (
...
-- Superseded wide projection and its experimental indexes are no longer needed.
DROP TABLE reelspan_private.movie_search;
