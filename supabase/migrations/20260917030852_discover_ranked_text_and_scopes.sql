-- Finish candidate ranking and batch location filtering; no result truncation.
...
metadata_matches AS MATERIALIZED (
 SELECT md.tmdb_id,
 CASE WHEN lower(coalesce(md.title,''))=i.q OR lower(coalesce(md.original_title,''))=i.q THEN 120
 WHEN position(i.q IN lower(coalesce(md.title,'')))>0 OR position(i.q IN lower(coalesce(md.original_title,'')))>0 THEN 110
 WHEN (position(i.q IN lower(coalesce(md.genres::text,'')))>0 OR position(i.q IN lower(coalesce(md.director::text,'')))>0 OR position(i.q IN lower(coalesce(md.cast_members::text,'')))>0) THEN 65
 WHEN position(i.q IN lower(coalesce(md.overview,'')))>0 THEN 45 ELSE 0 END score
 FROM public.movies md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND md.reelspan_discover_text LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT md.tmdb_id,
 CASE WHEN lower(coalesce(md.title,''))=i.q OR lower(coalesce(md.original_title,''))=i.q THEN 120
 WHEN position(i.q IN lower(coalesce(md.title,'')))>0 OR position(i.q IN lower(coalesce(md.original_title,'')))>0 THEN 110
 ...
 WHEN position(i.q IN lower(coalesce(md.overview,'')))>0 THEN 45 ELSE 0 END score
 FROM public.movies md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)<3 AND position(i.q IN md.reelspan_discover_text)>0
 ...
 WHERE (p_place_qid IS NOT NULL OR p_country_qid IS NOT NULL) AND ml.is_deleted=false
 AND (p_country_qid IS NULL OR ml.country_qid=p_country_qid)
 AND (
 (left(coalesce(p_place_qid,''),length('__city_unspecified__:'))<>'__city_unspecified__:'
  AND (p_place_qid IS NULL OR p_place_qid IN (ml.modern_place_qid,ml.city_qid,ml.admin1_qid,ml.country_qid,
       CASE WHEN ml.modern_place_qid IS NULL THEN ml.raw_place_qid END)))
 OR
 (p_place_qid='__city_unspecified__:'||ml.country_qid
 ...
 WHERE sm.is_deleted=false AND i.q='' AND p_concept_qid IS NULL
 UNION ALL
 SELECT r.movie_qid,r.score FROM relevance r CROSS JOIN input i WHERE (i.q<>'' OR p_concept_qid IS NOT NULL) AND r.score>0
), scored AS (
 ...
 AND (p_place_qid IS NULL AND p_country_qid IS NULL OR sm.movie_qid IN (SELECT movie_qid FROM scoped))
 ...
 AND (p_concept_qid IS NULL OR EXISTS (
