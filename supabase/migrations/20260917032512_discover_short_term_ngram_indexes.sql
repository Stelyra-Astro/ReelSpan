-- Trigram cannot prune one/two-character terms. Index literal characters and pairs.
CREATE FUNCTION reelspan_private.short_search_terms(input_text text) RETURNS text[]
LANGUAGE sql IMMUTABLE PARALLEL SAFE STRICT SET search_path='' AS $terms$
 WITH chars AS MATERIALIZED (
   SELECT c,lead(c) OVER () following FROM regexp_split_to_table(input_text,'') AS c
 )
...
$terms$;
REVOKE ALL ON FUNCTION reelspan_private.short_search_terms(text) FROM PUBLIC, anon, authenticated;
CREATE INDEX movie_search_short_terms ON reelspan_private.movie_search USING gin(reelspan_private.short_search_terms(search_text));
CREATE INDEX story_search_short_terms ON public.reelatlas_search_index USING gin(reelspan_private.short_search_terms(reelspan_discover_text));
...
metadata_matches AS MATERIALIZED (
 ...
 LEFT JOIN LATERAL (SELECT title,original_title,release_date,genres FROM public.movies WHERE tmdb_id=sm.tmdb_id LIMIT 1) md ON true
