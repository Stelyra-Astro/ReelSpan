-- Public catalog RPC. SECURITY DEFINER exposes only curated fields from private TMDB cache;
-- keep search_path empty and qualify every relation to prevent caller-controlled SQL resolution.
...
WITH input AS (
  ...
  SELECT sm.movie_qid,sm.legacy_id,sm.tmdb_id,sm.imdb_id,
         ...
         CASE
           ...
           WHEN EXISTS (SELECT 1 FROM public.story_movie_concept_tags t JOIN selected s ON s.concept_qid=t.concept_qid
                        WHERE t.movie_qid=sm.movie_qid AND t.is_deleted=false) THEN 105
           WHEN i.q<>'' AND EXISTS (SELECT 1 FROM public.story_movie_periods mp
                    WHERE mp.movie_qid=sm.movie_qid AND mp.is_deleted=false AND
                          (position(i.q IN lower(mp.period_name_en))>0 OR position(i.q IN lower(mp.period_name_zh))>0 OR
                           position(i.q IN lower(mp.period_labels::text))>0)) THEN 100
           WHEN i.q<>'' AND EXISTS (SELECT 1 FROM public.story_movie_locations ml
                    WHERE ml.movie_qid=sm.movie_qid AND ml.is_deleted=false AND
                          (position(i.q IN lower(ml.raw_place_name_en))>0 OR position(i.q IN lower(ml.raw_place_name_zh))>0 OR
                           position(i.q IN lower(ml.raw_place_labels::text))>0 OR
                           position(i.q IN lower(coalesce(ml.city_name_en,'')))>0 OR
                           position(i.q IN lower(coalesce(ml.country_name_en,'')))>0)) THEN 95
           ...
           WHEN EXISTS (SELECT 1 FROM selected s JOIN public.story_movie_periods mp
                    ON mp.movie_qid=sm.movie_qid AND mp.is_deleted=false
                    WHERE s.start_year IS NOT NULL AND s.end_year IS NOT NULL
                      AND mp.start_year<=s.end_year AND mp.end_year>=s.start_year) THEN 10
           ELSE 0 END AS score,
         i.q
