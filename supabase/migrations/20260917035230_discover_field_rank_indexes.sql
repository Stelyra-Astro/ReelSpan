-- Rank long-term hits directly from field indexes: no combined TOAST recheck followed by all-field scoring.
CREATE INDEX movie_search_title_trgm ON reelspan_private.movie_search USING gin(title extensions.gin_trgm_ops);
CREATE INDEX movie_search_original_title_trgm ON reelspan_private.movie_search USING gin(original_title extensions.gin_trgm_ops);
CREATE INDEX movie_search_genres_trgm ON reelspan_private.movie_search USING gin(genres extensions.gin_trgm_ops);
CREATE INDEX movie_search_director_trgm ON reelspan_private.movie_search USING gin(director extensions.gin_trgm_ops);
CREATE INDEX movie_search_cast_members_trgm ON reelspan_private.movie_search USING gin(cast_members extensions.gin_trgm_ops);
CREATE INDEX movie_search_overview_trgm ON reelspan_private.movie_search USING gin(overview extensions.gin_trgm_ops);
CREATE INDEX discovery_period_search_trgm ON public.reelatlas_search_index USING gin(period_search extensions.gin_trgm_ops);
CREATE INDEX discovery_place_search_trgm ON public.reelatlas_search_index USING gin(place_search extensions.gin_trgm_ops);
...
metadata_matches AS MATERIALIZED (
 SELECT md.tmdb_id, CASE WHEN md.title=i.q THEN 120 ELSE 110 END score FROM reelspan_private.movie_search md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND md.title LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT md.tmdb_id, CASE WHEN md.original_title=i.q THEN 120 ELSE 110 END score FROM reelspan_private.movie_search md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND md.original_title LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT md.tmdb_id, 65 score FROM reelspan_private.movie_search md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND md.genres LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT md.tmdb_id, 65 score FROM reelspan_private.movie_search md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND md.director LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT md.tmdb_id, 65 score FROM reelspan_private.movie_search md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND md.cast_members LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT md.tmdb_id, 45 score FROM reelspan_private.movie_search md CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND md.overview LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT md.tmdb_id,
 ...
 SELECT idx.movie_qid, 100 score FROM public.reelatlas_search_index idx CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND idx.period_search LIKE i.pattern ESCAPE chr(92)
 UNION ALL
 SELECT idx.movie_qid, 95 score FROM public.reelatlas_search_index idx CROSS JOIN input i
 WHERE i.q<>'' AND length(i.q)>=3 AND idx.place_search LIKE i.pattern ESCAPE chr(92)
 UNION ALL
