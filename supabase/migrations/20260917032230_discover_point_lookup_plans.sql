-- Bound per-candidate lookups prevent cardinality estimates from forcing full-table joins.
CREATE INDEX IF NOT EXISTS idx_reelspan_live_movie_qid_lower ON public.story_movies(lower(movie_qid)) WHERE is_deleted=false;
