...
- `SearchAndTip.swift`: observable latest-query coordinator and the three supported Tip Product IDs.
- `PurchaseManager.swift`: StoreKit 2 consumable manager for the three ReelSpan Tip products. It handles `Transaction.unfinished` and `Transaction.updates`, finishes only verified Tip transactions, and never creates an entitlement.
- `Tips.storekit`: local 0.99 / 2.99 / 4.99 test products. The app displays each live `Product.displayPrice`. This file is a project reference, not a Release bundle resource or default scheme override.
...
- `swift test --filter 'SearchAndTipTests|testMovieMetadata'`: 11 passed, 0 failed. Includes actual in-flight cancellation and repeated query generation guard, Tip Product ID/state validation and poster precedence/fallback checks, plus existing cache checks.
...
- Create the three ReelSpan consumables in App Store Connect for production/Sandbox purchases. Product availability depends on that configuration. For local manual testing: Xcode Edit Scheme → Run → Options → StoreKit Configuration → choose `ReelAtlas/Resources/Tips.storekit`. Do not select this file for production purchase verification.
...
5. Tip: check the three localized StoreKit prices, success, user cancellation, Ask to Buy pending and failed/unverified states. Tips must not change app access or create an entitlement.
